...
> **Standalone project follow-up (2026-09-21):** the independent repository now exists at `Chengy257/ZcodeGlobalQuotaClock`. Its documentation/implementation baseline is prepared on `main`, with package/CLI working name `gqclock`. This extraction inventory remains historical migration evidence; all new implementation authority has moved to the standalone repository.
