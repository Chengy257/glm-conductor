...
## 5. C 系列：ZC-02 Windows sleep（⛔ 挂起 → 用户裁决延后至 v2.3.1）

- 原计划：T3=00:52:19.916 武装后用户令机器睡眠跨过触发点、唤醒后观察补发/跳网格/丢弃行为。
- **2026-09-08 00:46 用户告知本机当前不能进行睡眠/关机操作——ZC-02 挂起为开放项。**
- **2026-09-08 ~09:00 用户终裁：ZC-02 延后至 v2.3.1 补做，v2.3.0 release gate 明示豁免
  （用户批准号：本轮对话明示"选2，ZC-02 后面补做吧，不影响本轮任务"）。**
- 缓解依据：ZC-01（App 重启）已覆盖"宿主进程消亡再恢复"的主路径语义；
  v2.3.1 补做后回填本节。T3 已被 park 覆盖（不再触发）。
