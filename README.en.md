...
> GLM-5.3 conducts, GLM-5.3-Flash implements, independent read-only reviewers audit — and the key contracts are enforced deterministically by runtime hooks, not left to model self-discipline; long-horizon tasks resume safely across quota windows.
...
Since v2, the plugin upgrades its key contracts from prompt text to **runtime enforcement**: out-of-scope changes can never silently pass the completion gate, completion claims without evidence are blocked, stale verification/review evidence expires automatically, and quota-exhausted tasks re-activate the same session through a persistent recurring bridge with bounded latency (durable SessionStart recovery as the fallback when the host is unavailable). All of this runs deterministically via plugin-distributed hooks and stdlib-only runtime modules.
...
**Long-horizon & quota (v2.2 quota continuity control loop)**
...
- **Dual-layer quota model (v2.2)** — the provider four-state (AVAILABLE / PRESSURE / EXHAUSTED / UNKNOWN) and the execution phase four-state (NORMAL / PRESSURE / DRAINING / BLOCKED) are two different dimensions: min remaining ≤ 20% rules DRAINING even when the provider reports AVAILABLE — the v2.1 24% misjudgment is structurally sealed; DRAINING forbids new implementation waves and only allows finish-up actions, BLOCKED freezes both; the continuation obligation (armed / wake_required / waiting_quota / degraded) is derived mechanically by the same pure decision function
- **Quota epoch & window materialization mechanics (v2.2)** — the meaningful event is no longer "a timer fired" but "a new executable quota epoch became available": epoch identity = a deterministic fingerprint over the window multiset `(kind, reset_at)` (status/percent excluded — a provider status flip never advances the epoch), `epoch_id = "glm:" + first 16 hex chars`; probe and executable boundaries are split (observation tightening and resume eligibility are never conflated). **Measured window mechanics**: at `reset_at` a window recovers to 100% (weekly precedence); the next `reset_at` materializes **only when a model call happens inside the new window** — pure queries never advance it; `reset_at` anchors to the materialization moment +5h00m01s; an idle session defers its window start (evidence: `docs/GLM-Conductor-v2.2-Phase0-Primer-Experiments.md`)
- **Quota watcher (v2.2, opt-in resident)** — `quota-watcher start|status|stop|once` launches a local resident observation process (poll-only, zero model calls, single-instance lock = the state file itself): while the session is dormant it independently maintains the true provider quota clock; ACTIVE/PASSIVE is re-evaluated every tick (ACTIVE only with a waiting_quota or auto-resume task; manual/notify stays PASSIVE — observable but never primes, never activates); observation intervals adapt by execution phase (NORMAL 1800s → DRAINING/BLOCKED 300s)
- **Window primer (v2.2, off by default)** — a new window materializes only through one minimal model call: the primer is the runtime's only control-plane model-call surface behind a fail-closed triple authorization gate (`primer_enabled` defaults to **false** + the auto-resume family + authorization source must be the user — manual/notify never prime); single-flight idempotency (at most one attempt per boundary, failed attempts recorded too); materialization evidence is only "the window identity change between two forced refreshes" — HTTP 200 and percent deltas are not evidence
- **Quota subscription & resume chain (v2.2)** — tasks subscribe to quota instead of owning a quota clock: the state's `quota_subscription` block records the registered epoch; activation is booked exactly once per epoch (QC-07, the control-plane `quota_epoch_advanced` event lands in `.glm-conductor/quota/events.jsonl`); the resume gate order is frozen as evaluate → transition → mark (on a mid-crash, prefer one lost booking over a stuck recovery); **the window budget is consumed exactly once, at the §15.1 resume commit point** (`record_quota_boundary_consumed`, idempotent per epoch; manual/notify never consume; arm/fire/create never consume); `quota-resume` exit codes 0/1/2/3 — 3 = durable-but-degraded (the transition may have landed; rerunning is idempotent-safe)
- **Authorized resume (v2.1 M5, closed out in v2.2)** — quota EXHAUSTED runs a deterministic transition chain: four-mode authorization (manual / notify / auto_once / until_done, escalations require user authorization recorded in state) + a self-sufficient wake prompt (host-verified: a wake continues the same session); the Persistent Wake Bridge (`wake-plan` adjudicates → the main session creates the host cron → `arm` records) is the stable path; budget exhaustion flips the task to `waiting_user`, and the resume entry point is always `quota-resume`
- **Activation transport (v2.2)** — "quota is available" and "a dormant session receives a new turn" are different problems; the latter is its own abstraction: `recurring_bridge` is the only stable transport (one tracked task ↔ one persistent recurring automation); `probe_then_hold` / `self_retiming` / `session_injector` are reserved (`arm` raises `TransportReservedError` rather than half-implementing); `transport-status` prints the frozen 12-key fact face
- **Stop-gate continuity health (v2.2)** — a position-0 check before the four-fold completion gate, bound to the dormant-handoff domain only (waiting_quota/waiting_user or PRESSURE/DRAINING + resumable + auto-resume): the trio is checked in dependency order — durable handoff (checkpoint + resume manifest), a registered quota subscription for the current epoch, and an armed activation transport; `create = forbidden` or a scheduled-task origin always degrades and allows instead of blocking forever (watcher absence/staleness is a degrade-only annotation)
- **Capability boundaries (stable)** — the watcher performs resident quota observation only and **never injects turns into a dormant session by itself**; same-session activation depends on the host Scheduled Task (auto-wakes may be missed while the host is closed or asleep — durable SessionStart recovery is the fallback); `recurring_bridge` is the only stable activation transport; `CronUpdate`/`CronDelete` success is not a correctness dependency; the single-controller (single orchestrating main session) model is unchanged, worker cap 4
- **Quota-aware continuity** — query Coding Plan usage (zero credential persistence), four-state evaluation; falls back to periodic liveness probing when unavailable; `/glm-conductor:quota` for on-demand diagnostics
...
The runtime quota/continuity CLI face (`python3 plugins/glm-conductor/runtime/cli.py <subcommand>`): `quota-resolve` / `quota-observe` / `quota-phase` / `quota-exhausted` / `quota-resume` (exit codes 0/1/2/3) / `wake-record` (v2.1 legacy, deprecated) / `wake-prompt` / `wake-plan` / `wake-status` / `wake-reconcile` / `transport-status` / `quota-watcher start|status|stop|once`. Host facts are session-supplied (e.g. `wake-reconcile` takes the CronList observation explicitly) — the runtime itself makes zero host `Cron*` calls; the completion-time bridge cleanup is a session-side single-attempt action (never retried).
...
**v2.2 Quota Continuity Control Loop (current dev line)** — upgrading the resume chain from agent discipline to a mechanical guarantee: the dual-layer quota model (provider four-state × execution phase, queryable via `quota-phase`) + quota epoch identity with dual boundaries (a rolling window, not a fired timer, constitutes a new epoch) + the opt-in resident quota watcher (`quota-watcher`, maintaining the true quota clock while the session sleeps) + the window primer (one minimal model call to materialize a new window, structurally off by default) + quota subscription with exactly-once-per-epoch activation booking (QC-07) + the §15.1 resume commit point as the sole consumption site + the activation transport abstraction (`recurring_bridge` stable) + the position-0 continuity-health trio in the completion gate. Measured window mechanics (user ruling 2026-09-03, evidence in `docs/GLM-Conductor-v2.2-Phase0-Primer-Experiments.md`): at `reset_at` a window recovers to 100% (weekly precedence); the next `reset_at` materializes only when a model call happens inside the new window — pure queries never advance it; an idle session defers its window start. Full suite: 2113 tests green (post-RH-01..05 baseline); `validate_plugin` 15/15. See CHANGELOG 2.2.0 (unreleased).
...
- **Quota awareness**: uses verified provider-api monitoring endpoints (zero credential persistence); no fabricated native quota interfaces, no hardcoded 5-hour resets; endpoint failures fall back to periodic liveness probing. The quota watcher is a manually started local process (`quota-watcher start` — never auto-started, never listens on the network); the window primer is structurally off by default (`primer_enabled=false` plus the triple authorization gate — manual/notify never trigger a model call)
