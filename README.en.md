...
- **Capability boundaries (stable)** — the watcher performs resident quota observation only and **never injects turns into a dormant session by itself**; same-session activation depends on the host Scheduled Task (auto-wakes may be missed while the host is closed or asleep — durable SessionStart recovery is the fallback); `recurring_bridge` is the only stable activation transport; `CronUpdate`/`CronDelete` success is not a correctness dependency; the single-task / single-controller model is unchanged, worker cap 4
