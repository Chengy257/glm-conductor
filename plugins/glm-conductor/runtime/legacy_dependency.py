...
【v2.3 遗留实现（legacy twin）】本模块由 runtime/dependency.py 整体
搬迁而来（git mv 保留历史，函数体零改动），仅供 legacy 执行路径
（runtime.dispatcher / runtime.task_manager 的既有就绪推导语义）
消费；v2.4 的新公开入口是纯 DAG 语义的 runtime/dependency.py。
本模块不是新兼容层——Phase 2 随 dispatcher 退役时一并删除，届时
新代码不得再导入。当前导入形如 `from runtime import
legacy_dependency as dependency`。
...
依赖：
    ...
    一个状态字面量；与 legacy_unit.py / state.py 均无循环导入）。
