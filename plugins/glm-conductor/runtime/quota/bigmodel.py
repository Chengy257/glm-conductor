#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""BigModel（open.bigmodel.cn）quota 监控适配器（v2 工作块 B5.2，
升级指南 §25/§37/§38）。
...
本模块只绑定 provider 标识 "bigmodel"；监控 host 经 _PROVIDER_HOSTS
注册表解析并受 ALLOWED_HOSTS 严格校验，代码路径不出现 host 字符串（仅本 docstring 说明端点）
（§25「不自行散布 host」）。

端点（实测，§25）：GET
https://open.bigmodel.cn/api/monitor/usage/quota/limit；鉴权头
Authorization: <API Key>（无 Bearer 前缀，与官方 glm-plan-usage
插件一致的已验证格式，§37 条款 8）。
...
用法：
    ...
    snapshot = BigModelQuotaProvider(api_key).fetch()  # 失败抛 QuotaProviderError
...
class BigModelQuotaProvider(HttpQuotaProvider):
    """BigModel quota provider（§24 抽象边界的一个实现）。
