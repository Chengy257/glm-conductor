#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GLM Conductor v2.1 execution policy CLI（M1 骨架，计划 §5）。

职责：
    以单行 JSON stdout 提供三个 policy 子命令，作为 execution_policy
    授权事实源的人工操作面。M1 只落数据层与 CLI 骨架——本模块不接线
    任何 hook / task_manager 消费方（那是 M2+ 的事）。
...
def main(argv=None) -> int:
    ...
    _TaskMissing（任务不存在）→ 1；其余意外异常 → 1（错误 JSON 含
    异常类型名，stdout 契约不破）。
    """
