...
【v2.3 遗留实现（legacy twin）】本模块由 runtime/work_unit.py 整体
搬迁而来（git mv 保留历史，函数体零改动），仅供 legacy 执行路径
（runtime.state / runtime.task_manager 的既有 work_units 运行时状态
语义）消费；v2.4 的新公开入口是静态节点的 runtime/work_unit.py。
本模块不是新兼容层——Phase 2 随 dispatcher 退役时一并删除，届时
新代码不得再导入。当前导入形如 `from runtime import legacy_unit
as work_unit`。
...
职责：
    ...
    legacy_dependency（原 runtime.dependency，B8.2）之上。
